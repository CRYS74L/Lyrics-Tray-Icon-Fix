...
#define DLL_NAME L"Lyrics Tray Icon Fix Hook v0.51.dll"
#define PSTF_HELPER_NAME L"Lyrics Tray Icon Fix PS Restore Helper v0.51.exe"
...
static void usage(void) {
    print_rules();
    out(L"\nUsage:\n");
    out(L"  Lyrics Tray Icon Fix v0.51.exe start   start PS Tray Factory route\n");
    out(L"  Lyrics Tray Icon Fix v0.51.exe stop    stop background hooks\n");
    out(L"  Lyrics Tray Icon Fix v0.51.exe apply   sync current rules once\n");
    out(L"  Lyrics Tray Icon Fix v0.51.exe status  show status\n");
    out(L"  Lyrics Tray Icon Fix v0.51.exe recover  internal bounded Shell recovery\n");
}
